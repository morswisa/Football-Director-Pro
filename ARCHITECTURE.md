...
- Trust, morale, board confidence, and reputation remain authoritative numeric fields in the engine/save model, but event copy intentionally exposes only qualitative reactions. Tests should verify internal numeric mutation separately from player-facing copy.
- `PersonAvatar` is a deterministic procedural SVG read model generated from player/manager IDs. It stores no image assets and does not change save schema; portrait consistency comes from the same seeded visual parameters.
...
- The Manager tab presents manager economics as mobile tiles/chips over the same engine data: current manager wage, contract, fire cost, status, attributes, and candidate expected wage/compensation are display read models only; hiring/firing mutations still go through `confirmFireManager` and `submitManagerHireOffer`.
...
- `PersonAvatar` exposes `thumb` and `portrait` variants. The portrait variant changes only presentation/crop with a taller SVG viewBox and bust details; it does not add saved data or change deterministic identity generation.
- The portrait generator follows a seeded component-variant architecture: the stable person id/name seed selects independent feature variants for face shape, hair mass, hairline, eyes, brows, nose, mouth, cheek planes, facial hair, glasses, scars, clothing, and background. All resulting SVG remains generated in React and no external portrait assets are stored or fetched.
- The mature portrait layer extends that architecture with larger visible archetype components: portrait archetype, light direction, jaw plane, ear shape, hair texture, sideburns, brow/eye-socket shadow, chin plane, age/skin detail, beard texture, and neck tendons. These are presentation-only read models inside `PersonAvatar`; they do not affect save schema, player data, or engine simulation.
- Archetype-level portrait components now include hand-shaped face paths, hair masses, temple shadows, shoulder silhouettes, collar geometry, and shirt-panel lines. The generator still renders one inline SVG per person, with unique React-derived SVG ids and deterministic seed choices; no external art assets, avatar APIs, or save migrations are involved.
- `EventEntityHeader` is the primary gameplay presentation for generated portraits. Player and manager events render the same deterministic `PersonAvatar` output in a larger dark sports-card frame with lightweight UI overlays; this changes only React presentation and does not create portrait state, assets, or engine coupling.
- `PersonAvatar` now includes deterministic highlight bias, hair-volume layers, hair shadow, cheek shadow, nose/face cast shadow, highlight-plane strokes, and asymmetric mouth paths. These are seeded presentation layers only; no portrait bitmap, API call, save field, or migration is introduced.
- The current portrait-depth layer adds seeded eye spacing, face-plane style, forehead plane, under-eye depth, mouth-plane shadow, hair-depth stroke sets, and a `portrait`-only zoom transform. These are all React/SVG presentation decisions inside `PersonAvatar`; compact `thumb` usage keeps the original crop density.
- Future portrait architecture is documented in `FACE_MODEL_RESEARCH.md`. The selected direction is a FaceVerse-inspired offline/cache pipeline: derive a deterministic `FaceGenome`, render portrait assets outside the app runtime, store only seed/genome metadata in saves, and fall back to the current SVG `PersonAvatar` if a generated bitmap is missing.
- The app runtime must not import PyTorch, PyTorch3D, Jittor, CUDA, or C++ 3DMM runtimes for portraits. Heavy model decoding/rendering belongs in tooling or a controlled generation service, not in the mobile web bundle.
- `src/game/portraits.ts` owns the renderer-agnostic `FaceGenome` model and deterministic `createFaceGenome` helper. It groups portrait identity into palette, geometry, hair, face, and detail channels.
...
- Git status verification on 2026-06-01 confirmed no architecture or code changes are pending locally: `main` matches `origin/main` at `432b766 Document Git upload verification`.
- Architecture review note, 2026-06-01: current friction is concentrated in shallow interfaces around the Continue event queue (`src/game/engine.ts`), event presentation (`src/components/game-client.tsx`), portrait rendering exported from the main game UI, store/persistence command choreography, and `z.any()` saved-event validation. No module shape has been changed yet; the temporary report is `/var/folders/md/n6drmgs14rs19ygxqqtmzzlr0000gn/T//architecture-review-20260601-051948.html`.
- Architecture review status: no seam has been changed from the report yet. The next step is candidate selection, then a grilling loop to define one deepened module before implementation.
- Architecture implementation is currently blocked, not incomplete due to missing exploration: the report is done, but the selected module/seam has not been chosen by the user.
- `src/components/person-avatar.tsx` is now the portrait renderer module. It renders `FaceGenome` data from `src/game/portraits.ts` and is imported by both the main game UI and `/portrait-lab`, removing the previous dependency from portrait-lab into the large `game-client.tsx` implementation.
- `src/game/schemas.ts` now owns the typed saved-event validation seam. `gameSaveSchema` still keeps broad world data permissive, but `currentEvent` and `eventQueue` validate through `gameEventSchema`, with typed payload modules for proposal, pending deal, financial snapshot, and season-history event data.
- `src/game/continue-loop.ts` is now the ContinueLoop module for queue mechanics. `engine.ts` still owns event scheduling and resolution policy, but the low-level implementation for queue normalization, dedupe, activation, handled-event tracking, and follow-up prioritization sits behind this module interface and is covered by `tests/continue-loop.test.ts`.
- `src/components/event-presentation.ts` is the renderless event presentation view-model module. It derives category, status, period, queue, tone, and note-display decisions from `GameSave/currentEvent`; `game-client.tsx` consumes that output while keeping event-specific interaction and layout in React.
- `src/game/career-commands.ts` is the Career command seam between UI/store adapters and engine mutations. It exposes creation/loading/import normalization plus command execution for Continue, event resolution, live-match finish, manager actions, facilities, stadium, and settings; `src/store/game-store.ts` persists the returned save and commits the returned message.
