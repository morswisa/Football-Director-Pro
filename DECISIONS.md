...
- Whole-template portrait variation is now preferred over piling more micro-details onto one base face. Each archetype may own its head path, hair mass, shoulder silhouette, collar, and shirt-panel geometry while sharing the same deterministic seed and color system.
- Important Continue decisions should show generated portraits at a card scale, not thumbnail scale. Player/manager subject headers use a sports portrait card so the face creator is visible in the primary loop, while compact thumbnails remain appropriate for feeds and dense lists.
- Mature SVG portraits should emphasize face-plane structure at normal mobile card size. Use deterministic highlight bias, hair volume, cheek shadows, cast shadows, and slight expression asymmetry before adding more tiny feature paths, because large lighting/silhouette layers make the portraits feel less childish without changing the save schema or relying on external assets.
- Portrait-card mode may crop/zoom the deterministic SVG more tightly than thumbnail mode. The primary Continue loop should show the head and facial planes clearly, while Roster/feed thumbnails can stay smaller and more compact.
- The next-generation face system should not run a full 3D face model inside Next.js or Capacitor. Use an offline/cache portrait-generation pipeline and store only deterministic portrait identity data in saves.
- FaceVerse is the selected architectural foundation for the next portrait pipeline because its parameter groups map cleanly to game needs: identity, expression, texture, lighting, pose, and camera. This is a foundation decision, not approval to ship FaceVerse model/checkpoint assets before license review.
- `nano_bfm`, `eos`, and `face3d` are reference projects for 3DMM math and pipeline structure; FLAME is a fallback for controllable head geometry if FaceVerse model rights or diversity are not acceptable.
...
- Continue-loop queue mechanics belong in `src/game/continue-loop.ts`, not inline in `engine.ts`. The current seam covers state normalization, duplicate suppression, queue activation, handled-event tracking, and prioritizing follow-up events queued during decision resolution.
- Continue event modal-shell presentation belongs in `src/components/event-presentation.ts`, not inline in `game-client.tsx`. The module owns event category grouping, status labels, period/queue labels, tone classes, and note visibility while React remains responsible for rendering the card and event-specific content.
- Career mutation choreography belongs in `src/game/career-commands.ts`, not directly in the Zustand store. The command seam returns `{ save, message }` for career actions; the store remains the adapter responsible for loading/persisting saves and applying state.
